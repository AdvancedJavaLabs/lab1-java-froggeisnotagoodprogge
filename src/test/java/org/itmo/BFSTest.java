...
public class BFSTest {
    ...
    public void bfsTest() throws IOException {
        ...
        try (FileWriter fw = new FileWriter("tmp/results.txt")) {
            for (int i = 0; i < sizes.length; i++) {
                ...
                long parallelTime = executeParallelBfsAndGetTime(g, null);
        ...
        int threadsTestIndex = Math.max(6, sizes.length - 1);
        ...
        for (int tc : threadCounts) {
            long t = executeParallelBfsAndGetTime(gThreads, tc);
            threadsX.add(tc);
    ...
    private long executeParallelBfsAndGetTime(Graph g, Integer threads) {
        long startTime = System.currentTimeMillis();
